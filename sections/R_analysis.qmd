...
all **only available for specific time windows**, in particular the
column labeled 'SNB policy rate' is only **available since June 2019**.
So we have to decide which (or which combination) would be best for our
analysis.
...
the other. Furthermore it shows very different behaviour regarding
...
After checking the dates in data, we did find major policy rate changes.

For example, interste rate target average was lower from 3.25 to 2.25 in
2001-09-01, in 2008_11-01 lowered from 2.5 to 1 and continuously lowered
in the following months, and in 2023-03-01 the interest rate was rised
from 1 to 1.5, which is part of a continous interest rate hike for
months.

These 3 events are reflection of substantial economic interventions, so
they are not errors.
...
**shapiro-wilk Test: rejects** the null hypothesis of **normally
distributed residuals**, however with sample size n=302, the non
nomrality may **not have great impact**.

**Cook's distance:** several events had great influence on the
coefficient, they are **structural events in reality**, so they are
**not errors**.

**Durbin Watson Test:** **no significant autocorrelation** in residuals.

All residual tests proves that our linear model is valid and has noo
data quality issues.

So **why** the small R-squared?

It doesn't necessarily mean that the model is poor, the **reasons**
could be that:

-   inflation is affected by **many factors**.

-   time series data , in economic events and shocks can be **noisy.**

-   linear relationship is w**eak but statistically significant** and
...
**common** that a change in the policy rate has effect on inflation
rates **10 months later**, but we wonder **why only Lag_1**0 has this
effect and nearby months don't? The overall credibiity of the lag effect
is **questionable**.

**Our conclusion**: As only 6% of variation is explained by the model
and the two significant lags are against theory, we are **not
...
(at lag 5 and lag6). This means, that the SNB **policy rate change in
autumn 2022** **influenced** the Swiss inflation rates.
...
economic slow down in Europe, global e**nergy crisis**, and a **25% draw
...
models in futures studies could be good.
