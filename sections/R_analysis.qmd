...
Looking at different available columns in the policy rate data from SNB
website. 
...
Breusch-Pagan test for Constant variance, with null hypothesis that
Residuals are homoscedastic shows, that there is no significant evidence
of heteroskedasticity in the linear regression model.
...
Lag 1 and lag 10 are significant (p-value \< 0.05), but R squared
(0.06249, adjusted 002187) is very low.

We are not sure if a lag of 10 months makes sense from a logical point
of view.
...
(+0.50). But R squared is very low.
...
Interpretation: ar1 to ar5 are highly significant at the 95% confidence
interval. The negative values of the coefficients reveals that a
positive change in the time series in the previous period leads to a
negative change in the subsequent period.
...
inflation in our model. But remember: Granger causality is not proof of
true causation, it only indicates predictive ability.
