...
In this Dataset, “From 13 June 2019, the SNB policy rate is applied.
...
was proven. The result also gives us more confidence that both Datasets
reflect the economic reality, although Dataset 2 appeared more
consistent overall.
...
with readers.
=======
prices – SNB and SFSO core inflation rates” \[Cons00b\]. In Dataset 1,
the threshold is optimal to be used as the interest rate policy target,
and before 2009 there is no threshold data in Dataset 1, we have used
the special rate. From 13 June 2019, the special rate was based on the
current SNB policy rate plus a surcharge of 50 basis points. The special
rate always amounted to at least 0.5 percent”\[Inte00\]. Additionally,
we determined that the SARON fix rate is the market reaction to the
policy target, so it shouldn’t be used as the policy rate to be
analyzed.
...
data. So, we researched more, and we did find a second data set
(afterwards called data set 2). It only contains 2 types of data to be
...
With the second data set, we’ve also done a VAR to evaluate if there is
causality between policy rate/target to inflation, or inflation to the
policy decisions, which with both data sets, no causality was proven.
This also gives us more confidence that both data sets represent the
reality well, even though the data set 2 seemed more consistent.

After trying, we've chosen data set 2 and applied all analysis to it as
the final result. However the data selection process was a valuable
experience to share with readers.
>>>>>>> 0394fe200960f39456567a3f89057d8bc1700799
